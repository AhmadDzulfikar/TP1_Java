...
public class MainMenu {
    ...
    // Memeriksa apakah String terdiri dari karakter-karakter yg bisa direpresentasikan angka
    ...
    // method untuk memeriksa apakah semua elemen di array dapat disimpan dengan valid
    public static boolean simpanMenu(String[] menuArray) {
        for (String menu : menuArray) {
            String[] makananDanHarga = menu.split(" ");
            if (!isDigit(makananDanHarga[makananDanHarga.length - 1])) {
                return false;
            }
        }
        return true;
    ...
    // method untuk memisahkan string menjadi dua (nama makanan & harganya)
    public static String[] splitMenu(String menu) {
        String[] makananDanHarga = menu.split(" ");
        String harga = makananDanHarga[makananDanHarga.length - 1];
        String namaMenu = menu.substring(0, menu.length() - (harga.length() + 1));
        return new String[] {namaMenu, harga};
    }

    // Method membuat restoran beserta menambah menunya
    ...
    while (true) {
        ...
        boolean restoTerdaftar = isRestoranTerdaftar(nama); // untuk mengecek apakah restoran sudah terdaftar
        ...
        input.nextLine();
        String[] menuArray = new String[jumlahMakanan];
        for (int i = 0; i < jumlahMakanan; i++) {
            System.out.print(" ");
            String inputMenu = input.nextLine();
            menuArray[i] = inputMenu;
        }
        if (simpanMenu(menuArray)) {
            tambahRestoran(nama, menuArray); // Menggunakan method baru untuk menambahkan restoran ke dalam daftar
    ...
    // Method untuk mengecek apakah restoran sudah terdaftar
    public static boolean isRestoranTerdaftar(String namaRestoran) {
        for (Restaurant resto : restoList) {
    ...
    // Method untuk menambahkan restoran ke dalam daftar
    public static void tambahRestoran(String namaRestoran, String[] menuArray) {
        ArrayList<Menu> listMenu = new ArrayList<Menu>();
        for (String menu : menuArray) {
            String[] makananDanHarga = splitMenu(menu);
            String namaMakanan = makananDanHarga[0];
            double harga = Double.parseDouble(makananDanHarga[1]);
            listMenu.add(new Menu(namaMakanan, harga));
        }
        Restaurant newRestaurant = new Restaurant(namaRestoran);
        newRestaurant.setMenu(listMenu);
